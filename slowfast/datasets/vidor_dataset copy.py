...
class Vidor(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        """
        # Loading frame paths.
        self._image_paths, self._video_idx_to_name = vidor_helper.load_image_lists(
            cfg, is_train=(self._split == "train")
        )
        # Loading annotations for boxes and labels.
        boxes_and_labels = vidor_helper.load_boxes_and_labels(
        ...
        ) = vidor_helper.get_keyframe_data(boxes_and_labels)
    ...
    def print_summary(self):
        ...
        print_log("Number of videos: {}".format(len(self._image_paths)))
        total_frames = sum(
            len(video_img_paths) for video_img_paths in self._image_paths
        )
        print_log("Number of frames: {}".format(total_frames))
        print_log("Number of key frames: {}".format(len(self)))
        print_log("Number of boxes: {}.".format(self._num_boxes_used))
    ...
    def __getitem__(self, idx):
        ...
        image_paths = [self._image_paths[video_idx][sec_idx]]
        ...
        for i, box_labels in enumerate(labels):
            # VidOR label index starts from 0.
